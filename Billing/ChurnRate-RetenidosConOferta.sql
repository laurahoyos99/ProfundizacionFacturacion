...
INNER JOIN RetenidosConOferta r ON c.NOMBRE_CONTRATO=r.Contrato
WHERE TIPO_ORDEN = "DESINSTALACION" AND ESTADO = "FINALIZADA" 
AND FECHA_FINALIZACION IS NOT NULL AND FECHA_APERTURA IS NOT NULL
AND FECHA_FINALIZACION > r.FechaTiquete AND DATE_DIFF ( FECHA_FINALIZACION,r.FechaTiquete, DAY) <= 120
...
FROM RetenidosConOferta r LEFT JOIN Churners c ON c.NOMBRE_CONTRATO=r.Contrato
